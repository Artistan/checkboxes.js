...
(function ($) {
    ...
    Checkboxes.prototype.max = function (max) {
        if (max > 0) {
            // Enable max.
            var instance = this;
            this.$context.bind('click.checkboxes.max', ':checkbox', function () {
                if (instance.$context.find(':checked').length === max) {
                    instance.$context.find(':checkbox:not(:checked)').attr('disabled', true);
                } else {
                    instance.$context.find(':checkbox:not(:checked)').attr('disabled', false);
                }
            });
        } else {
            // Disable max.
    ...
    Checkboxes.prototype.range = function (enable) {
        if (enable) {
            ...
            this.$context.bind('click.checkboxes.range', ':checkbox', function (event) {
    ...
    $(document).bind('click.checkboxes.data-api', '[data-toggle^=checkboxes]', dataApiClickHandler);
    $(document).bind('ready.checkboxes.data-api', dataApiDomReadyHandler);
